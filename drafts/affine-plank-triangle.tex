...
equality if and only if each is the central third of its median; and (v) a
...
masses $1-2p_j$. We isolate the transport method behind (i)--(v) as a
\emph{uniform-marginal witness measure} and give a dimension-agnostic necessary
condition for its existence (concurrence of the mid-level lines); for cyclic triples
the condition is also sufficient, giving a complete characterization. Finally we
...
(\S\ref{sec:median}) and the weighted-perimeter Theorem~\ref{thm:wper}
(\S\ref{sec:concur}), which extends the sharp bound from the medians to a
two-parameter family of generic concurrent triples.
Proposition~\ref{prop:concur} (\S\ref{sec:concur}) is the dimension-agnostic
necessary condition, and Corollary~\ref{cor:iff} the resulting characterization for
cyclic triples; Proposition~\ref{thm:nogo} (\S\ref{sec:nogo}) is the normalizer
obstruction.
...
maps the medial triangle to itself; Theorem~\ref{thm:wper} and
Corollary~\ref{cor:iff} hold for it verbatim. Non-cyclic (mixed) role assignments,
and the rigidity of the equality case for $p$ off the centroid, remain open
...
Corollary~\ref{cor:iff} characterizes the cyclic (and anti-cyclic) role
assignments. For the remaining \emph{mixed} role assignments of a concurrent
triple, decide whether a uniform-marginal witness measure exists; and determine the
equality (rigidity) case of Theorem~\ref{thm:wper} for $p$ off the centroid ---
does $\sum\rw=1$ force a tiling, and is it ever attained?
\end{problem}
\begin{problem}
