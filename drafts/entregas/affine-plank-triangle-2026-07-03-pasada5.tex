...
introduce the \emph{transport defect} $D_K(u_1,\dots,u_n)$: the least $D$ such
that some probability measure on $K$ has all its $u_i$-marginals bounded by
$D\cdot\mathrm{Leb}$. Every covering of $K$ by planks parallel to these
directions then satisfies $\sum\rw\ge1/D_K(u)$, and $D_K$ obeys a strong
linear-programming duality that produces certified two-sided bounds. On the
triangle we determine exactly when the best constant is certified: for three
pairwise non-parallel directions, $D=1$ --- i.e.\ a witness measure with uniform
marginals exists --- if and only if the mid-level lines of the directions are
concurrent; in the concurrent case we exhibit the witness measure, the tight
covering, and its uniqueness, extending the sharp median case to a
two-parameter family of rigid configurations. The same framework organizes
everything else we prove: the classical bound $\sum\rw\ge1/d$ is the statement
that the uniform measure certifies $D\le d$ on every $d$-dimensional body; the
facet directions of the $d$-simplex have $D=(d+1)/2$ exactly, calibrating the
method's limits; and near the concurrent locus $D$ has an explicit linear
modulus. All theorems bound coverings by planks in specified direction families
on a fixed body, or characterize witness measures; none resolves the conjecture
...
(iii) $D_K$ obeys a strong linear-programming duality
...
$\sum\rw\ge\tfrac{27}{29}>4\sqrt3-6$ for coverings by planks parallel to any
cyclic triple with $\max_i|\tau_i-\tfrac12|\le\tfrac1{60}$
(Corollary~\ref{cor:beyond}) --- a bound valid only for these restricted
direction families, whereas the constant of~\cite{BY26} is uniform over all
coverings (Remark~\ref{rem:beyond}).
...
\begin{theorem}[strong duality]\label{thm:duality}
...
$\psi_i$ range over bounded Borel functions.
\end{theorem}
...
bound exactly --- if and only if $p^\ast\in T_u$.
\end{itemize}
\end{proposition}
...
The degenerate case $\delta_c=0$ cannot separate the two defects:
mid-level lines of a genuine triple cannot concur outside the triangle.

\begin{lemma}[no external concurrence]\label{lem:external}
Let $u$ be a pairwise non-parallel triple of directions on $\Delta$ and extend
the mid-level lines $\{u_i=\tfrac12\}$ affinely to the plane of $\Delta$. If
the three lines have a common point $q$, then $q\in\Delta$. Consequently
$\delta_c(u)=0\iff\delta(u)=0\iff D(u)=1$.
\end{lemma}

\begin{proof}
Work in barycentric coordinates on the plane ($\sum_jq_j=1$, signs
unrestricted). A direction with value $0$ at vertex $j_0$, $1$ at vertex
$j_1$ and $s\in[0,1]$ at the remaining vertex $j_s$ takes at $q$ the value
$q_{j_1}+s\,q_{j_s}$. Its mid-line passes through $q$ iff
$q_{j_1}+s\,q_{j_s}=\tfrac12$; since $q_{j_1}+q_{j_s}=1-q_{j_0}$, such an
$s\in[0,1]$ exists iff $\tfrac12$ lies (weakly) between $q_{j_1}$ and
$1-q_{j_0}$, i.e.\ iff
\[
\bigl(q_{j_0}-\tfrac12\bigr)\bigl(q_{j_1}-\tfrac12\bigr)\ \ge\ 0 .
\]
Moreover the ordered patterns $(j_0,j_1)$ and $(j_1,j_0)$ produce flipped,
hence parallel, directions ($u\mapsto1-u$ exchanges them). So the number of
pairwise non-parallel directions whose mid-lines pass through $q$ is at most
the number of \emph{unordered} pairs $\{j,k\}$ with
$(q_j-\tfrac12)(q_k-\tfrac12)\ge0$. All three pairs qualify only when no two
coordinates of $q$ lie strictly on opposite sides of $\tfrac12$, i.e.\ when
all $q_j\le\tfrac12$ or all $q_j\ge\tfrac12$. The latter contradicts
$\sum_jq_j=1$; the former forces
$q_j=1-q_k-q_l\ge1-\tfrac12-\tfrac12=0$ for each $j$, i.e.\ $q\in\Delta$.
Thus for $q\notin\Delta$ at most two of the three pairs qualify, and three
pairwise non-parallel mid-lines cannot concur at $q$.

For the consequence: $\delta_c=0$ means the center lies on $\Pi$
(Proposition~\ref{prop:wedge}(ii)). The mid-lines of $u_1,u_2$ meet at a
single point $q$ (non-parallel), and
$c_0=\sum_ic_iu_i(q)=\tfrac12(c_1+c_2)+c_3u_3(q)$ together with
$c_0=\tfrac12\sum c_i$ gives $u_3(q)=\tfrac12$ ($c_3\ne0$): the three
mid-lines concur at $q$, which lies in $\Delta$ by the first part, so
$\delta(u)=0$ and $D(u)=1$ (Corollary~\ref{cor:done}). The converses are
trivial.
\end{proof}

\begin{remark}\label{rem:wedgemoment}
Proposition~\ref{prop:wedge} and Lemma~\ref{lem:external} give the hierarchy
\[
\frac1{1-2\delta_c(u)}\ \le\ \frac1{1-2\delta(u)}\ \le\ D(u),
\]
where the left quantity is what single-knot wedges certify, the middle is the
moment bound, the two coincide iff $p^\ast\in T_u$, and they vanish
together ($\delta_c=0\iff\delta=0$). In every exactly computed example of
this paper the two defects coincide --- for the facet triple both equal
$\tfrac16$ and the wedge certificate ($c=(1,1,1)$, $c_0=1$,
$\theta=\tfrac23$) is precisely the facet $\psi$ displayed above; for the
sandwich triple of Remark~\ref{rem:sandwich} both equal $\tfrac1{450}$ ---
but we do not claim $\delta_c=\delta$ in general. Lower bounds beyond
Proposition~\ref{prop:moment} would have to come from richer $\psi$
(piecewise-linear with several knots), which Theorem~\ref{thm:duality}
guarantees is the \emph{only} thing they can come from
(cf.\ Problem~\ref{prob:defect}).
\end{remark}
...
give certified \emph{lower} bounds. We stress that the skeleton LP computes
only an upper bound for the infimum over all of $\mathcal P(\Delta)$; it
computes $D(u)$ itself precisely when some optimal measure is skeletal
(Remark~\ref{rem:skeleton}), which we cannot certify off the concurrent
locus. For example, for the non-concurrent cyclic parameter
$\tau=(\tfrac{13}{25},\tfrac12,\tfrac12)$:
\[
\tfrac{225}{224}\ \le\ D(u_\tau)\ \le\ \tfrac{112}{111}.
\]
The lower bound is the wedge certificate of Proposition~\ref{prop:wedge}
(dependency $c=(75,76,74)$, $c_0=113$, $\delta_c=\tfrac1{450}$), which here
coincides with the moment bound ($\delta=\delta_c=\tfrac1{450}$); the upper
bound is the optimal piecewise-uniform edge-skeleton measure. Which bound, if
either, is exact is open: closing the gap requires either a non-skeletal
measure beating $\tfrac{112}{111}$ or an admissible $\psi$ beyond single-knot
wedges beating $\tfrac{225}{224}$. The ancillary scripts contain the exact
computations.
\end{remark}
...
triangle (as it does at the facets --- in every dimension, by
Theorem~\ref{thm:facetd} --- and, trivially, on the concurrent locus)? In
particular, is $\sup_uD(u)=\tfrac32$, attained at the facet triple? What is
known: $\delta(u)\le\tfrac16$ for every family
(Theorem~\ref{thm:centroid}), so the moment bound never certifies more than
$\tfrac32$, and the supremum over pairwise non-parallel triples lies in
$[\tfrac32,2]$ (Theorems~\ref{thm:facetdefect} and~\ref{thm:oned}). By
Theorem~\ref{thm:duality}, a triple with $D(u)>\tfrac32$ would have to be
exposed by an admissible $\psi$ strictly richer than the single-knot wedges
of Proposition~\ref{prop:wedge}. A positive answer to the sup-question would
give the universal bound $\sum\rw\ge\tfrac23$ for coverings of the triangle
by planks parallel to \emph{any} three directions --- well below
$4\sqrt3-6$, so of structural rather than quantitative interest.
\end{problem}
\begin{problem}
