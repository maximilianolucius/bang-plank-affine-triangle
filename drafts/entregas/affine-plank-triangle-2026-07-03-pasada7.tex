...
non-parallel directions a uniform-marginal witness measure exists ($D=1$) if
and only if the mid-level lines concur; for every concurrent triple we exhibit
the witness, the tight covering with $\sum\rw=1$, and prove its uniqueness --- a
two-parameter family of rigid configurations extending the median case. We also
settle, partly by an exact-rational computation, one \emph{non-concurrent}
triple, the first for which the three-plank bound is established. Calibrations:
the uniform measure gives $D\le d$ on any $d$-dimensional body, and the
$d$-simplex facets have $D=(d+1)/2$. No result resolves the conjecture for the
triangle, let alone the plane.
\end{abstract}
\maketitle
...
\emph{A first non-concurrent case.} Beyond the concurrent locus, where the
transport method no longer certifies the sharp constant, \S\ref{sec:problems}
frames the covering constant $C_\Delta$ as the remaining open object and, for
one explicit non-concurrent triple, establishes $C_\Delta=1$ (the three-plank
bound) --- in part by an exact-rational computation with an independent
certificate checker (Theorem~\ref{thm:sandwichbang}). This is a single triple,
not the family, and does not bear on Conjecture~\ref{conj:bang}.
...
We settle Problem~\ref{prob:covconst} at one non-concurrent triple: the
\emph{sandwich} $\tau_0=(\tfrac{13}{25},\tfrac12,\tfrac12)$ of
Remark~\ref{rem:sandwich} ($\Pi=\tfrac{13}{100}\ne\tfrac{12}{100}=Q$, so it is
non-concurrent by Theorem~\ref{thm:canon}). This is the first non-concurrent
triple for which the three-plank bound is established. The proof is by hand
except for one finite emptiness check, isolated below and discharged by an
exact-rational computation with an independent certificate checker
(Appendix~\ref{app:ancillary}); we mark the boundary explicitly.

Throughout, a covering is by three planks $P_i=\{u_i\in[l_i,h_i]\}$, one per
direction, $r_i=h_i-l_i\ge0$. The first tool is the two-direction case as a
local instrument.
...
Let $T(\tau_0)$ be the binary tree grown from the root $\mathcal C$ by pruning
a box when any of \textup{(P1)--(P4)} holds and otherwise bisecting its widest
coordinate at the midpoint. Then $T(\tau_0)$ is finite, and a proper balanced
covering of $\Delta$ with $\sum r\le1$ exists if and only if $T(\tau_0)$ has an
unpruned leaf.
\end{lemma}
\begin{proof}
Finiteness: on a small enough box one predicate always fires (bisection drives
either $\min\sum r>1$, or the enlarged configuration to still miss a fixed
uncovered region), so the recursion terminates; leaves are exactly the pruned
boxes. By Lemma~\ref{lem:prunes} a pruned box holds no proper covering with
$\sum r\le1$, and every such covering lies in some leaf, which is then
unpruned.
\end{proof}

\begin{theorem}[$C_\Delta(\tau_0)=1$; \emph{partially computer-assisted}]
\label{thm:sandwichbang}
For the sandwich $\tau_0=(\tfrac{13}{25},\tfrac12,\tfrac12)$, every covering of
$\Delta$ by three planks, one parallel to each direction, satisfies
$\sum_i\rw\ge1$, with equality if and only if the covering is trivial (one
plank is all of $\Delta$, the other two empty). In particular
$C_\Delta(\tau_0)=1$.
\end{theorem}
\begin{proof}
Partition coverings into (i) extreme ($\max r_i\ge1-w_0$), (ii) some plank
empty, and (iii) proper balanced. Case (i) is Theorem~\ref{thm:extreme}
($\sum r\ge1$, equality trivial); case (ii) is Theorem~\ref{thm:twodir}
($\sum r\ge1$). For case (iii), by Lemma~\ref{lem:reduction} the assertion
``every proper balanced covering has $\sum r>1$'' is equivalent to
\emph{$T(\tau_0)$ has no unpruned leaf}: any such covering lies in a leaf,
which is neither (P4) (proper) nor (P3) (it covers), so (P2) --- excluded,
balanced --- or (P1), whose closed box has $\min\sum r>1$, giving
$\sum r>1$. \emph{That $T(\tau_0)$ is empty is a finite verification in exact
rational arithmetic}, carried out and re-verified from a certificate by an
independent checker sharing no code with the search
(Appendix~\ref{app:ancillary}); the tree has $812{,}651$ leaves, all pruned.

Equality: (i), (ii) give equality only trivially; case (iii) is strict; and no
proper two-plank tight covering exists --- minimizing $|I_i|+|I_j|$ over
$2$-plank coverings gives $\tfrac{31}{25}$ for the pairs containing $u_1$ and
$\tfrac54$ for $(u_2,u_3)$, both $>1$ (exact, \texttt{r15\_equality.py}). Hence
$\sum\rw=1$ forces a trivial covering.
\end{proof}

\begin{remark}[the human/machine boundary]\label{rem:boundary}
Every step above is proved by hand except the single assertion
``$T(\tau_0)$ has no unpruned leaf'', a finite emptiness check of an explicitly
defined tree. That check is \emph{exact} --- rational arithmetic throughout,
no floating point, independent of hardware --- but not human-scale
($\approx1.6\times10^6$ nodes); this is what makes
Theorem~\ref{thm:sandwichbang} partially computer-assisted, and its validity
rests on the independent checker of Appendix~\ref{app:ancillary}, not on
trusting the search (Appendix~\ref{app:ancillary} also records an independent
second search producing a different tree with the same verdict). This settles
one triple, not the family: Problem~\ref{prob:covconst} remains open.
\end{remark}
...
Theorem~\ref{thm:sandwichbang} ($C_\Delta(\tau_0)=1$ at the sandwich
$\tau_0$) reduces, by the human Lemmas~\ref{lem:twoplank}--\ref{lem:reduction}
...
of Lemma~\ref{lem:noncover}(a). SHA-256 (first $16$ hex):
searcher \texttt{b2468aa9acc91f4f}, certificate \texttt{ead66ff2cfd547f8}
($12{,}062{,}803$ bytes), checker \texttt{90a25ce8ea059abe}. As independent
